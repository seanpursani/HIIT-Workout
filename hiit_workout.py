...
def start_hiit(length, hiit_set, level):
    ...
    """
        ...
        Args:
            length [integer]: 3 or 6 representing the amount of workout sets
...
def get_exercises(level, focus):
    ...
    # load JSON file
    with open(r"C:\Users\seanp\Software Dev\PYTHON PROJECTS\CODEINPLACE PROJECTS\CodeInPlace\HIIT_Workout_Generator\exercises.json") as data:
    ...
    for category in exercises: 
        for difficulty in exercises[category].items():
            ...
            # difficulty must be equal to user defined level
            if difficulty[0] == level: 
                
                # Loop until the given exercise is a unique item
                while True:
